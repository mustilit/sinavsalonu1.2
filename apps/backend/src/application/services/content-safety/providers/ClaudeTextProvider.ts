...
/** Token başına yaklaşık maliyet (USD / 1M token) */
...
const SYSTEM_PROMPT = fs.readFileSync(
  path.join(__dirname, '../prompts/text-moderation.tr.md'),
  'utf-8',
);
...
function resolveCost(
  ...
  // Env override ile geçersiz kılınabilir
  ...
  const rates = Number.isFinite(overrideInput) && Number.isFinite(overrideOutput)
    ? { inputPer1M: overrideInput, outputPer1M: overrideOutput }
    : (Object.entries(COST_TABLE).find(([key]) => modelName.includes(key))?.[1] ?? DEFAULT_COST);
...
@Injectable()
export class ClaudeTextProvider implements IModerationTextProvider {
  private readonly apiKey: string;

  constructor(
    /** Claude model adı — AdminSettings'ten gelir (UseCase tarafından inject edilir) */
  ...
  async analyze(text: string, _tenantId: string): Promise<Layer2Result> {
    ...
    const Anthropic: any = await (async () => {
      try {
        ...
        throw new AppError(
          ...
          '@anthropic-ai/sdk paketi kurulu değil. `npm install @anthropic-ai/sdk` çalıştırın.',
          500,
        );
      }
    })();
    ...
    try {
      response = await client.messages.create({
        ...
        system: [
          {
            type: 'text',
            text: SYSTEM_PROMPT,
            // Prompt cache — system prompt büyük ve değişmez
        ...
        messages: [
          {
            role: 'user',
            content: `Lütfen aşağıdaki metni değerlendir:\n\n${text}`,
          },
        ],
      });
    } catch (err: any) {
      logger.error('[ClaudeTextProvider] API hatası', { error: err?.message });
      throw new AppError('CLAUDE_API_ERROR', `Claude API hatası: ${err?.message}`, 502);
    }
    ...
    let parsed: any;
    try {
      // JSON bloğu içindeyse ayıkla
      const jsonMatch = rawText.match(/```json\s*([\s\S]*?)```/);
      parsed = JSON.parse(jsonMatch ? jsonMatch[1] : rawText);
    } catch {
      logger.warn('[ClaudeTextProvider] JSON parse başarısız, MANUAL_REVIEW döndürülüyor', {
        rawText,
      });
      // Parse edilemezse güvenli tarafta kal
